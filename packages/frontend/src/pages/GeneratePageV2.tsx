                ...
                <div className="space-y-4">
                  {result.plans.map((plan, index) => {
                    ...
                    const endDateFormatted = endDate
                      ? new Date(endDate).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })
                      : null;
            ...
            >
              ...
              <div className="p-4 border-b bg-[var(--paper-cream)]">
                ...
                <div className="flex gap-3 mt-3 flex-wrap">
                  <span className="sticker sticker-mint">{viewingPlan.totalDays}일</span>
                  <span className="sticker sticker-gold">⏱ {viewingPlan.totalEstimatedHours}시간</span>
                  {viewingPlan.dailyQuests.length > 0 && (
                    <span className="sticker sticker-pink">
                      🏁 {new Date(viewingPlan.dailyQuests[viewingPlan.dailyQuests.length - 1].date).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })} 완료
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <div className="space-y-3">
                  {viewingPlan.dailyQuests.map((quest, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`p-3 rounded-lg border ${isWeekend ? 'bg-[var(--highlight-pink)] border-pink-200' : 'bg-[var(--paper-cream)] border-[var(--paper-lines)]'}`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2 py-0.5 rounded ${isWeekend ? 'bg-pink-200 text-pink-700' : 'bg-[var(--highlight-blue)] text-[var(--ink-blue)]'}`}>
                              Day {quest.day}
                            </span>
